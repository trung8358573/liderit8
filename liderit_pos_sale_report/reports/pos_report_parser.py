...
class POSReportParser(report_sxw.rml_parse):
    ...
    def get_sale_days(self, data):
        ...
        obj = self.pool.get('pos.order').browse(self.cr, self.uid, pos_orders[0], context=None)
        first_date = parser.parse(obj.date_order).date()
        sum_tax = 0
        sum_total = 0
        bank_amount = 0
        cash_amount = 0
        for order in pos_orders:
            ...
            order_date = parser.parse(obj1.date_order).date()
            if first_date == order_date:
