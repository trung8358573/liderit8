openerp.style_aristos_quick_sale = function (instance) {
    ...
    instance.web.sale.QuickSaleAction = instance.web.Widget.extend({
        ...
        events: {
            ...
            // autocomplete customer-name in renderElement
            ...
            'click .cart-row-item .product-qty input.qty':'clear_quantity',
            ...
            'click .cart-row-item .product-price input.price_unit':'clear_quantity', 
...
//            Barcode Scan Method Code
        ...
        scan_handler: function(e){
            ...
            if (e.which === 8) {
                ...
                //console.info("Code wich 8: ",self.code);
            }
        ...
        prepare_orderline: function(product){
            ...
            lines[0] = line;
            console.info("Valor de lines al prepare_order_line: ",lines[0]);
            $('#product-search').attr('placeholder','Search product with barcode.');
            $('#product-search').val('');
            // $('#product-search').autocomplete('close').val('');
            self.add_orderline(lines);
            
        },
        add_orderline: function(lines){
            var self = this;
            ...
            console.info("Valor de lines al add_orderline: ",lines);
            $.each(lines, function(index, line) {
                    console.info("Valor de cada line al add_orderline: ",line);
                    line['cid'] = self.cid += 1;
                    if(pricelist_id){
        ...
        render_orderlines: function(lines){
            console.info("Valor de lines al render_orderline: ",lines);
            var self = this;
            $.each(lines, function(index, line) {
            ...
            // self.update_summary();
            self.update_order(lines);
        },
        update_order: function(nlines){
            var self = this;
            ...
            if (sale_order_id == 0){
                ...
                new instance.web.Model('sale.order').call('update_so_lines',[nlines,sale_order_id,global_discount],{}, {async: true})
                    .then(function(sale_order_id){
                    if(sale_order_id){
                        self.sale_order_id = sale_order_id;
                    }
                    });
                // $.each(nlines, function(index, line) {
                //     line['sale_order'] = sale_order_id;
                //     line['global_discount'] = global_discount;
                //     // async : true para variantes cuando hay un sale_order
        ...
        render_orderline: function(line){
            ...
            // self.update_summary();
            self.update_orderone(line);
        },
        update_orderone: function(line){
            var self = this;
            var global_discount = Number($('#in-global-discount').val());
            var sale_order_id = self.sale_order_id || 0;
            if (sale_order_id == 0){
        ...
        fill_sale_order: function(sale_order){
            ...
            if(sale_order && sale_order.lines){
                ...
                sale_order.lines.map(function(line){
                    //console.info("Valor de line en fill_sale_order: ",line);
     ...
     //        }
        ...
        updatenoup_order: function(lines){
            var self = this;
            var global_discount = Number($('#in-global-discount').val());
            var sale_order_id = self.sale_order_id || 0;
            if (sale_order_id == 0){
                ...
                line['global_discount'] = global_discount;
                // async : true para variantes cuando hay un sale_order
                new instance.web.Model('sale.order').call('update_so_lines',[lines], {}, {async: false})
                .then(function(sale_order_id){
                if(sale_order_id){
                    self.sale_order_id = sale_order_id;
                }
   ...
   //  	},
	    ...
	    add_variants: function(){
            $('#add-variants').attr("disabled", true);
	        var self = this;
            ...
            _.each($('div.product-variants table tr'), function(el, index){
                ...
                if(qty && qty > 0){
                    ...
                    console.info("VAlor de line para nueva lines: ",line);
                    lines.push(line);
                    variant_copy +=1;
            ...
            console.info("Total de lines: ",lines);
            self.add_orderline(lines);
