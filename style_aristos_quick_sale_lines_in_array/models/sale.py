...
class SaleOrder(models.Model):
    ...
    @api.model
    def update_so_lines(self, lines, order_id, global_discount):
        logger.error('Valor de line en update so lines %s'%lines)
        ids_entrega=[]
        discount = 0
        warehouse=''
        th_tipoent = self.env['st.line.type']
        ...
        for line in lines:
            ...
            line_env = self.env['sale.order.line']
            product_obj = self.env['product.product'].browse(line.get('product').get('id'))
            sol_exist = line_env.search([('quick_cid','=',line.get('cid')),('order_id','=',order_id)], limit = 1)
            if sol_exist:
                vals={
                    'product_id':line.get('product_id'),
                    'product_uom_qty': line.get('quantity'),
                ...
                # copiamos los de alguna linea anterior
                # pero tenemos que prever que el tipo de pedido no sea sin impuestos
                th_sale = self.env['sale.order'].browse(order_id)
                old_line = line_env.search([('order_id','=',order_id)], limit = 1)
