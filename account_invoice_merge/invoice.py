...
class account_invoice(models.Model):
    ...
    def _get_invoice_line_key_cols(self):
        fields = [
            ...
            'product_id', 'account_id', 'account_analytic_id', 'order_lines'
        ]
    ...
    def do_merge(self, keep_references=True, date_invoice=False):
        ...
        * Invoice lines are exactly the same except for the quantity and unit
        ...
        def make_key(br, fields):
            ...
            for field in fields:
                ...
                if (isinstance(field_val, browse_record) and
                   field != 'invoice_line_tax_id' and
                   field != 'order_lines'):
                    field_val = field_val.id
                elif isinstance(field_val, browse_null):
                    field_val = False
                elif (isinstance(field_val, list) or
                      field == 'invoice_line_tax_id' or
                      field == 'order_lines'):
        ...
        for account_invoice in draft_invoices:
            ...
            #_logger.error('##### AIKO ###### Valor en merge invoices de origins: %s' % origins)
            #_logger.error('##### AIKO ###### Valor en merge invoices de client_refs: %s' % client_refs)
            ...
            if not invoice_infos:
                ...
                if account_invoice.name and keep_references:
                    #_logger.error('##### AIKO ###### Valor en merge invoices de name: %s' % invoice_infos['name'])
                    if len(str(invoice_infos['name']))<150:
                        invoice_infos['name'] = \
                            (invoice_infos['name'] or '') + \
                            (' %s' % (account_invoice.name,))
                if account_invoice.origin and \
                        account_invoice.origin not in origins:
                    #_logger.error('##### AIKO ###### Valor en merge invoices de origin: %s' % invoice_infos['origin'])
                    if len(str(invoice_infos['origin']))<150:
                ...
                if account_invoice.reference \
                    ...
                    #_logger.error('##### AIKO ###### Valor en merge invoices de reference: %s' % invoice_infos['reference'])
                    if len(str(invoice_infos['reference']))<150:
        ...
        for invoice_key, (invoice_data, old_ids) in new_invoices.iteritems():
            ...
            # _logger.error('##### AIKO ###### Valor en new invoice de invoice_data: %s' % invoice_data)
        ...
        # make link between original stock.picking and new invoice
        pick_obj = self.env['stock.picking']
        for new_invoice_id in invoices_info:
            todos = pick_obj.search(
                    [('invoice_id', 'in', invoices_info[new_invoice_id])])
            todos.write({'invoice_id':  new_invoice_id})
