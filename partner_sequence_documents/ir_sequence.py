...
class CustomIrSequence(osv.osv):
    ...
    def _is_custom(self, cr, user, ids, field_name, arg, context=None):
        ...
        for sequence in self.browse(cr, user, ids, context=context):
            partner_ids = partner_obj.search(cr, user, ['|','|','|','|','|','|',
                                                        ...
                                                        ('purchase_quo_id','=',sequence.id),
                                                        ('sale_seq_id','=',sequence.id),
                                                        ('sale_quo_id','=',sequence.id),
    ...
    _columns = {
        'is_custom': fields.function(_is_custom,type="boolean",string='Es personalizada',help="Indica si la secuencia está asociada a algún partner (Cliente o Proveedor)",store=True),
    }
