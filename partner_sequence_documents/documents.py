    ...
    def create(self, cr, uid, values, context = None):
        ...
        if 'partner_id' in values:
            ...
            if partner.custom_sequence:
                if not partner.purchase_quo_id:
                    if not partner.purchase_seq_id:
                        raise except_orm('Advertencia','No está definida una secuencia de pedidos de compras para este proveedor')
                    else:
                        values['name'] = self.pool.get('ir.sequence').next_by_id(cr, uid, partner.purchase_seq_id.id, context = context)
                else:
                    values['name'] = self.pool.get('ir.sequence').next_by_id(cr, uid, partner.purchase_quo_id.id, context = context)
            else:
                if values.get('name', '/') == '/':
        ...
        if 'partner_id' in values:
            ...
            if partner.custom_sequence:
                if not partner.sale_quo_id:
                    ...
                    values['name'] = sequence_obj.next_by_id(cr, uid, partner.sale_quo_id.id, context = context)
            else:
                if values.get('name', '/') == '/':
    ...
    def create(self, values):
        ...
        if partner.custom_sequence:
            ...
            if journal.type == 'purchase':
                ...
                diario_id = journal.search([('invoice_sequence_id','=', partner.in_invoice_seq_id.id)])
                ...
                diario_id = journal.search([('invoice_sequence_id','=', partner.in_refund_seq_id.id)])
            elif journal.type == 'sale':
                if not partner.out_invoice_seq_id:
                    raise except_orm('Advertencia','No está definida una secuencia de facturas de venta para este cliente')
                number = sequence_obj.next_by_id(self.env.cr, self.env.uid, partner.out_invoice_seq_id.id)
                diario_id = journal.search([('invoice_sequence_id','=', partner.out_invoice_seq_id.id)])
            elif journal.type == 'sale_refund':
                if not partner.out_refund_seq_id:
                    raise except_orm('Advertencia','No está definida una secuencia de facturas rectificativas de venta para este cliente')
                number = sequence_obj.next_by_id(self.env.cr, self.env.uid, partner.out_refund_seq_id.id)
                diario_id = journal.search([('invoice_sequence_id','=', partner.out_refund_seq_id.id)])
            values['reference'] = number
            if len(diario_id)>0:
                values['journal_id']= diario_id[0].id
