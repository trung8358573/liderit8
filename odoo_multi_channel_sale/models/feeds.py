...
class WkFeed(models.Model):
    ...
    def open_mapping_view(self):
        ...
        duplicity_config=self.env['multi.channel.sale.config'].get_default_fields(dict())
        domain = [
    ...
    # @api.model
    # def get_tag_id(self, store_tag_id,channel_id):

    #     message = ''
    #     tag_id = None
    #     match = channel_id.match_tag_mappings(store_tag_id)
    #     if match:
    #         tag_id = match.odoo_tag_id
    #     else:
    #         feed = channel_id.match_tag_feeds(store_tag_id)
    #         if feed:
    #             res = feed.import_tag(channel_id)
    #             message += res.get('message', '')
    #             mapping_id = res.get('update_id') or res.get('create_id')
    #             if mapping_id:
    #                 tag_id = mapping_id.odoo_tag_id
    #         else:
    #             message += '<br/>Tag Feed Error: No mapping as well tag feed found for %s .' % (
    #                 store_tag_id)
    #     return dict(
    #         tag_id=tag_id,
    #         message=message
    ...
    def get_order_partner_id(self, store_partner_id,channel_id):
        ...
        context['no_mapping'] = 1
        _type = 'contact'
        if not self.customer_is_guest:
            match = channel_id.match_partner_mappings(
                store_partner_id, _type)

            _logger.error('#Valor de match en get_order_partner %r'%match)
            if match:
                partner_id = match.odoo_partner
            else:
                partner_id = self.create_partner_contact_id(partner_id,channel_id)
            
            # partner_invoice_id = channel_id.match_partner_mappings(
            #     self.invoice_partner_id, 'invoice').odoo_partner
            # if not partner_invoice_id:
            #     partner_invoice_id = self.create_partner_invoice_id(partner_id,channel_id)
            partner_invoice_id = partner_id

            if self.same_shipping_billing:
                partner_shipping_id = partner_invoice_id
            else:
                partner_shipping_id = channel_id.match_partner_mappings(
                    self.shipping_partner_id, 'delivery').odoo_partner
                if not partner_shipping_id:
                    partner_shipping_id = self.create_partner_shipping_id(
                        partner_id,channel_id)
        else:
            partner_id = self.with_context(context).create_partner_contact_id(partner_id,channel_id)
            partner_invoice_id = self.with_context(context).create_partner_invoice_id(partner_id,channel_id)
            if self.same_shipping_billing:
                partner_shipping_id = partner_invoice_id
            else:
                partner_shipping_id = self.with_context(context).create_partner_shipping_id(partner_id,channel_id)
    ...
    @api.model
    def create_partner_invoice_id(self, partner_id,channel_id):
        partner_obj = self.env['res.partner']
        _type = 'invoice'
        ...
        if not self._context.get('no_mapping'):
            store_id = self.invoice_partner_id
            channel_id.create_partner_mapping(erp_id, store_id, _type)
        return erp_id

    @api.model
    def create_partner_shipping_id(self, partner_id,channel_id):
        partner_obj = self.env['res.partner']
        _type = 'delivery'
        ...
        erp_id = partner_obj.create(vals)
        if not self._context.get('no_mapping'):
            store_id = self.shipping_partner_id
            channel_id.create_partner_mapping(erp_id, store_id, _type)
        return erp_id

    @api.model
    def create_partner_contact_id(self, partner_id,channel_id):
        partner_obj = self.env['res.partner']
        _type = 'contact'
...
class ProductFeed(models.Model):
        ...
        for variant in variant_objs:
            ...
            if not exists:
                ...
                if variant.list_price:
                    exists.product_name.wk_extra_price = float(
                        variant.list_price.replace(',','')) - float(template_id.list_price.replace(',',''))

                    price =float(variant.list_price.replace(',','')) #- template_id.list_price
    ...
    def get_variant_extra_values(self, template_id, variant,channel_id):
        ...
        if variant.list_price and template_id:
            vals['wk_extra_price'] = float(
                variant.list_price.replace(',','')) - (template_id.list_price.replace(',',''))
    ...
    def wk_change_product_price(self, product_id, price, channel_id):
        ...
        # pricelist_item = channel_id.pricelist_name.version_id[0].items_id.filtered(
        ...
        # else:
        #     price_version_id = self.env['product.pricelist.version'].create({
        ...
        #          'price_version_id':price_version_id.id,
        #         'fixed_price': (price),
        #     })
    ...
    def import_product(self,channel_id):
        ...
        list_price = list_price and float(list_price.replace(',','')) or 0
        ...
        if state == 'done':
            ...
            if match:
                ...
                match.write({'default_code':vals.get('default_code'),'barcode':barcode})
                ...
                if len(variant_lines):
                    ...
                    for variant_id in template_id.product_variant_ids:
                        variant_vals = variant_id.read(['default_code','ean13'])[0]
                        self.wk_change_product_price(
                        ...
                        match_product = channel_id.match_product_mappings(
                            store_id, 'No Variants', default_code=variant_vals.get('default_code'), barcode=variant_vals.get('ean13'))
                            ...
                            channel_id.create_product_mapping(
                                template_id, variant_id, store_id, 'No Variants',
                                 {'default_code': variant_vals.get('default_code'),
                                        'barcode':variant_vals.get('ean13')})
                update_id = match
                    ...
                    if template_id:
                        create_id = channel_id.create_template_mapping(
                            template_id, store_id,
                             {'default_code': vals.get('default_code'), 'barcode': vals.get('ean13')})
