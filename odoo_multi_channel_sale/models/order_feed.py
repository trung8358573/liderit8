...
class OrderFeed(models.Model):
    ...
    def _get_order_line_vals(self, vals,carrier_id,channel_id):
        ...
        if line_price_unit:line_price_unit = line_price_unit.replace(',','')
        ...
        if line_ids:
            for line_id in self.env['order.line.feed'].browse(line_ids):
                ...
                if line_price_unit:line_price_unit = float(line_price_unit.replace(',',''))
            ...
            if product_id:
                if line_price_unit:line_price_unit = float(line_price_unit.replace(',',''))
                if line_product_uom_qty:line_product_uom_qty = float(line_product_uom_qty.replace(',','')) or 1
                line = dict(
                    name=line_name or '',
                    price_unit=float(line_price_unit),
                    product_id=product_id.id,
                    is_delivery = line_source =='delivery',
                    product_uom_qty=float(line_product_uom_qty),
    ...
    def  get_taxes_ids(self, line_taxes,channel_id):
        if line_taxes:
            ...
            if line_taxes:
                ...
                _logger.error('#Order Feed get_taxes_ids valor de line_taxes %r'%line_taxes)
                for tax in line_taxes:
                    # ahora tax['rate'] nos informa del id de la tax almacenado en el mappin
                    ...
                    #     name = tax['name']
                    # else:
                    #     name = channel+"_"+str(channel_id.id)+"_"+str(float(tax_rate))
                    ...
                    domain = [('store_tax_value_id','=',tax_rate)]
                    tax_rec = channel_id._match_mapping(tax_mapping_obj, domain)
                    _logger.error('#Order Feed get_taxes_ids valor de tax_rec %r'%tax_rec)

                    if tax_rec:
                        # tax_rec.tax_name.price_include = inclusive
                        tax_list.append(tax_rec.tax_name.id)
                    else:
                        raise osv.except_osv(_('Error'),_("Error : Tipo de impuesto no reconocido. "))
                        # tax_dict={
                        #   'name'            : name,
    ...
    def import_order(self,channel_id):
        ...
        _logger.error("Valor de store_partner_id %r"%store_partner_id)
        if store_partner_id:
            ...
            if partner_id and partner_invoice_id and partner_shipping_id:
                _logger.error("Valor de partner_id en vals %r"%partner_id.id)
        ...
        if state=='done':
            ...
            # comment = vals.pop('comment','')
            comment = vals.pop('note','')
            if comment:
                vals['note'] = comment
        ...
        vals.pop('website_message_ids','')
        vals.pop('message_follower_ids','')
        if channel_id.crm_team_id.id:
            vals['section_id'] = channel_id.crm_team_id.id
        vals['user_id'] = channel_id.order_user_id.id
        ...
        if match and match.order_name:
            ...
            _logger.error("Vals antes de if state done %r"%vals)
            if state == 'done':
                try:
                    order_state = vals.pop('order_state')
