...
from openerp.addons.odoo_multi_channel_sale.tools import chunks,  DomainVals, ReverseDict
...
MAPPINGMODEL = {
    ...
    # 'st.type_material': 'channel.tag.mappings',
...
class MultiChannelSale(models.Model):
    ...
    def _get_count(self):
        ...
        # tags = self.env['channel.tag.mappings'].search_count(domain)
    ...
    # default_tag_id = fields.Many2one(
    #     comodel_name='st.type_material',
    #     help="""Default tag used for imported products.""",
    #     string='Tag',
    #     default=lambda self: self.env['st.type_material'].search([], limit=1)
    # )
    ...
    crm_team_id = fields.Many2one(
        ...
        string='Sales Team',
        # default=lambda self: self.env['crm.case.section'].search([], limit=1),
    ...
    order_user_id = fields.Many2one(
        ...
        string='Commercial',
        # default=lambda self: self.env['crm.case.section'].search([], limit=1),
        help='Commercial user used for imported order.',
    )
    ...
    def _match_obj(self, obj, domain=None, limit=None):
        ...
        if limit:
            obj.search(new_domain, limit=limit)
    ...
    def match_partner_mappings(self, store_id = None, _type='contact',domain=None, limit=1):
        ...
        _logger.error("Valor de store_id en match_partner %r"%store_id)
        if store_id:
            map_domain +=[('store_customer_id', '=', store_id)]
        _logger.error("Valor de map_domain en match_partner %r"%map_domain)
    ...
    # @api.model
    # def match_tag_mappings(self, store_tag_id=None,odoo_tag_id=None, domain=None, limit=1):
    #     map_domain = self.get_channel_domain(domain)
    #     if store_tag_id:
    #         map_domain += [('store_tag_id', '=', store_tag_id)]
    #     if odoo_tag_id:
    #         map_domain += [('odoo_tag_id', '=', odoo_tag_id)]
    #     return self.env['channel.tag.mappings'].search(map_domain, limit=limit)

    # @api.model
    # def match_tag_feeds(self, store_id=None,domain=None,limit=1):
    #     map_domain = self.get_channel_domain(domain)
    #     if store_id:map_domain  += [('store_id', '=', store_id)]
    #     return self.env['tag.feed'].search(map_domain, limit=limit)
    ...
    def default_multi_channel_values(self):
        return self.env['multi.channel.sale.config'].sudo().get_default_fields(dict())
    ...
    def match_odoo_template(self, vals,variant_lines):
        ...
        barcode =  vals.get('barcode')
        if barcode:
            record = Template.search([('ean13', '=', barcode)], limit=1)
        if not record:
    ...
    def match_odoo_product(self, vals, obj='product.product'):
        ...
        barcode =  vals.get('barcode')
        if barcode:
            record = oe_env.search([('ean13', '=', barcode)], limit=1)
        if not record:
            default_code =  vals.get('default_code')
            ir_values = self.default_multi_channel_values()
            if ir_values.get('avoid_duplicity') and default_code:
    ...
    # @api.model
    # def create_tag_mapping(self, erp_id, store_id):
    #     self.ensure_one()
    #     vals = dict(
    #         store_tag_id=store_id,
    #         odoo_tag_id=erp_id.id,
    #         tag_name=erp_id.id,
    #     )
    #     channel_vals = self.get_channel_vals()
    #     vals.update(channel_vals)
    ...
    def sync_order_feeds(self, vals,**kwargs):
        ...
        try:
            ...
            # if kwargs.get('tag_vals'):
            #     message += self.sync_category_feeds(
            #         tag_vals,channel_vals= channel_vals)[0].get('message','')
    ...
    # @api.one
    # def sync_tag_feeds(self, vals, **kwargs):
    #     channel_vals = kwargs.get('channel_vals') or self.get_channel_vals()
    #     res= self.create_model_objects('tag.feed', vals, extra_val = channel_vals)
    #     message = res.get('message','')
    #     data = res.get('data')
    #     if data:
    #         for data_item in data:
    #             import_res = data_item.import_tag(self)
    #             message += import_res.get('message', '')
    #     return dict(
    #         message = message
    #     )
    ...
    def read_website_image_url(image_url):
        ...
        try:
            ...
            if res.status_code == 200:
                data = binascii.b2a_base64(str(res.content))
        except Exception as e:
    ...
    # @api.model
    # def _match_create_product_tag(self, vals):
    #     match = self.match_tag_feeds(store_id= vals.get('store_id'))
    #     feed_obj = self.env['tag.feed']
    #     update = False
    #     if match:
    #         vals['state'] = 'update'
    ...
    @api.model
    def get_channel_category_id(self,template_id,channel_id):
        ...
        return channel_id.match_category_mappings(domain=domain).mapped('store_category_id')
