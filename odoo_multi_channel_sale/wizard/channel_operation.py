...
class ChannelOperation(models.TransientModel):
	...
	#                 raise Warning("""Last Updated Before Date Should be
	#                     greater than the Last Updated After Date!""")
	#         if (fields.Datetime.from_string(self.to_date) >
	#             fields.Datetime.from_string(fields.Datetime.now())):
	#             raise Warning("""Last Updated Before Date Should
        ...
        if channel_id.auto_evaluate_feed:
            for feed_id in (create_ids+update_ids):
                try:
                    ...
                    _logger.info("=post_feed_import_process==%r===="%(e))
