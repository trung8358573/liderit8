...
class MrpPropertyGroup(models.Model):
    ...
    text_description = fields.Boolean('Propiedad en descripción', default=False)
    ...
    def price_property_ids_changed(self):
        ...
        if self.product_id:
            for prop in self.env['mrp.property'].with_context(ctx).browse(
                ...
                # agregamos la descripcion con las properties
                if prop.group_id.text_description:
                    # self.name += ' '+ prop.name
                    self.name = ' '.join([self.name, prop.name])
