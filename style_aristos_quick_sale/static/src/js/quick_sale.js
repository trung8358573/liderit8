...
//            Barcode Scan Method Code
        ...
        start: function() {
            ...
            if(this.sale_order_id && this.sale_order_id.length > 0){
                // reindexamos antes de cargar
                var sale_order = self.index_sale_order(this.sale_order_id[0]);
        ...
        // funcion para reindexar las lineas del sale order antes de cargarlas
        index_sale_order: function(sale_order_id){
            var self = this;
            if(sale_order_id){
                new instance.web.Model('sale.order').call('reindex_sale_lines',[sale_order_id], {}, {async: false})
        ...
        fill_sale_order: function(sale_order){
            ...
            if(sale_order && sale_order.sale_order_id && sale_order.lines){
                self.update_so = false;
                sale_order.lines.map(function(line, index){
                    index += 1;
                    if(index === sale_order.lines.length){
                        self.update_so = true;
                    }
                    self.add_orderline(line);
                });
        ...
        add_orderline: function(line){
            ...
            // console.info("Line quick cid en add orderline: ",line.quick_cid);
            if(line.quick_cid && line.quick_cid!=0){
                line['cid'] = line.quick_cid;
                self.cid = line.quick_cid;
            }else{
                line['cid'] = self.cid += 1;
            }
            // console.info("Valor de cid en add orderline: ",line.cid);
     ...
     //        });
	    ...
	    remove_line_by_id: function(line_id){
	    	...
	    	// self.update_summary();
            // en lugar de quitar ponemos en la linea el texto 'linea eliminada' para no perder el cid de las lineas
            // self.cart_lines.map(function(line){
            //     if(line && line.cid == line_id){
            //         line.description = 'LINEA ELIMINADA';
            //         line.quantity = 0;
            //         line.price_unit = 0;
            //     }
            //     self.rendernoup_orderline(line);
        ...
        change_quantity: function(event){
            var self = this;
            var line_id = $(event.currentTarget).parent().parent().data('line-id');
            console.info("Valor de line_id en change_qty: ",line_id);
            ...
            if(line_id){
                self.cart_lines.map(function(line){
                    if(line && line.cid == line_id){
                        line.quantity = qty || 1;
                        console.info("Valor de line para el render en change_qty: ",line);
                        self.render_orderline(line);
                    }
                });
            }else{
        ...
        update_summary: function(){
            ...
            lines.map(function(line){
                ...
                if(($.inArray(line.product.id, product_id_list) == -1)){
                    // if(line.quantity>0){total_products += 1;}
