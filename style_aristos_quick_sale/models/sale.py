...
class SaleOrder(models.Model):
    ...
    def update_so_line(self, line):
        ...
        if line.has_key('global_discount') and line.get('global_discount') and line.get('global_discount')<>0:
        ...
        product_obj = self.env['product.product'].browse(line.get('product').get('id'))
    ...
    @api.model
    def reindex_sale_lines(self, sale_order_id):
        logger.error('Valor de sale_order_id en el reindex %s'%sale_order_id)
        order_lines = self.env['sale.order.line'].search([('order_id','=',sale_order_id)],order="id asc")
        th_quick_cid = 1
        for line in order_lines:
            line.quick_cid = th_quick_cid
            th_quick_cid += 1
        return self.load_sale_order_by_id(sale_order_id)
